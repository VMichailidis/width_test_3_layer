...
#include "data/model_gradients.h"
...
bool test_ce() {
    ...
    int val[BATCH_test];
    T dout_hw[BATCH_test][IN_DIM], dout_sw[BATCH_test][IN_DIM];
    T pred_hw[BATCH_test][OUT_DIM], pred_sw[BATCH_test][OUT_DIM];
    T loss_hw[BATCH_test], loss_sw[BATCH_test];
    ...
    // Backprop(grad, net, in, din);
    cout << "circuit" << endl;
    CE_train_wrapper(loss_hw, pred_hw, dout_hw, grad1, grad2, L1, L2, in, val); 
